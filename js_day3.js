...
function kredit(result, bulan){
    console.log('Harga kredit selama ${bulan} bulan :');
    
    const kredit = result/bulan;
    const bayar = [];

    for (let j = 0; j < bulan; j++){
        bayar.push(kredit);
        console.log('Bulan ke ${j+1} : Rp.${bayar[j]}');
    }
